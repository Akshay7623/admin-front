...
const SessionBcone = () => {
    ...
    const setTarget  = ()=>{
     ...
     if(parseInt(targetAmount) >0){
      fetch(`${Hosturl}/api/setactivetarget`,{
        method:'post',
        body:JSON.stringify({server:'bcone',target:targetAmount}),
        headers:{
         ...
         if(finalData.message === 'success'){
          fetch(`${Hosturl}/api/getalltargets`,{
            method:'post',
            body:JSON.stringify({server:'bcone'}),
            headers:{
              'Content-Type':'application/json',
              'authorization':`Bearer ${token}`
            }
     ...
     if(num === ''){
      ...
      if(remainMin === 0){
        if(remainSec <=30 && remainSec>5){
          ...
          method:'post',
          body:JSON.stringify({server:'bcone',number:num}),
          headers:{
            'Content-type':'application/json',
            'Authorization':`Bearer ${token}`
      }
    ...
    const genImage = ()=>{
      ...
      if(x === cp){
        MakePrediction('bcone',cp,color,x,true).then(()=>{
          setPredictionResult([{server:'bcone',period:cp,predict:color,result:''}]);
        });
      }else{
        MakePrediction('bcone',cp,color,x,false).then((data)=>{
          console.log(data.data);
          if(checked){
            setPredictionResult([...data.data,{server:'',period:'',predict:'white',result:'Profit'}]);
          }else{
            setPredictionResult([...data.data,{server:'bcone',period:cp,predict:color,result:''}]);
          }
        });
      }
  ...
  return (
    <>
       ...
       <section className="dashboard">
        ...
        <NavLink to="/sessions/bcone"><div><button className="server-btn active-server">Bcone</button></div></NavLink>
