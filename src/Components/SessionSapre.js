...
const SessionSapre = () => {
    ...
    const setTarget  = ()=>{
     ...
     if(parseInt(targetAmount) >0){
      fetch(`${Hosturl}/api/setactivetarget`,{
        method:'post',
        body:JSON.stringify({server:'sapre',target:targetAmount}),
        headers:{
         ...
         if(finalData.message === 'success'){
          fetch(`${Hosturl}/api/getalltargets`,{
            method:'post',
            body:JSON.stringify({server:'sapre'}),
            headers:{
              'Content-Type':'application/json',
              'authorization':`Bearer ${token}`
            }
     ...
     if(num === ''){
      ...
      if(remainMin === 0){
        if(remainSec <=30 && remainSec>5){
          ...
          method:'post',
          body:JSON.stringify({server:'sapre',number:num}),
          headers:{
            'Content-type':'application/json',
            'Authorization':`Bearer ${token}`
      }
    ...
    const genImage = ()=>{
      ...
      if(x === cp){
        MakePrediction('sapre',cp,color,x,true).then(()=>{
          setPredictionResult([{server:'sapre',period:cp,predict:color,result:''}]);
        });
      }else{
        MakePrediction('sapre',cp,color,x,false).then((data)=>{
          console.log(data.data);
          if(checked){
            setPredictionResult([...data.data,{server:'',period:'',predict:'white',result:'Profiit'}]);
          }else{
            setPredictionResult([...data.data,{server:'sapre',period:cp,predict:color,result:''}]);
          }
        });
      }
    ...
    <>
      ...
      <div className="flex flex-row justify-around">
       ...
       <section className="dashboard">
        ...
        <NavLink to="/sessions/sapre"><div><button className="server-btn active-server">Sapre</button></div></NavLink>
